...
def find_path(maze, x0,y0,x1,y1):
    ...
    while not(x_curr == x1 and y_curr == y1):

            
            neighbors = maze.reachable_neighbors(x_curr,y_curr)
            distances = []
            for neighbor in neighbors:
                x,y = neighbor[0],neighbor[1]
                distances.append((distance(x1,y1,x,y),x,y))
            #choose neighbor with minimum distance to endpoint
            next_neighbor = min(distances)
            x_curr,y_curr = next_neighbor[1],next_neighbor[2]
            path.append((x_curr,y_curr))            
    return path 
